...
export class CatalogService {
	...
	private readonly stalenessConfig: StalenessConfig = {
		categoryMaxAgeMs: 24 * 60 * 60 * 1000, // 24 hours
	...
	async getAllCategories(): Promise<CategoryDocument[]> {
		...
		// 2. If empty, SEED the database by scraping the homepage
		if (count === 0) {
			this.logger.log(
	...
	async getCategory(
		...
		// 2. Check logic: "Does not exist" OR "Is Stale"
		...
		if (needsScraping) {
			...
			// Trigger live scrape and save to DB
	...
	// ===========================================================================
	// SCRAPE & SYNC LOGIC (Categories)
	// ===========================================================================

	/**
	 * Scrapes the Homepage to get ALL categories and bulk-saves them.
	 */
			...
			updateOne: {
				...
				update: {
					$set: {
						...
						// Ensure imageUrl is saved if your schema supports it (add it to schema if needed)
					},
					$setOnInsert: {
						product_count: 0, // Will be updated when we scrape specific category
						parent_id: null,
					},
				},
				upsert: true,
	...
	private async scrapeAndSaveCategory(
		...
		if (!scrapeResult.success || !scrapeResult.data) {
			...
			if (!existingCategoryId) {
				// If it's a new category request and scraping failed, we can't return anything.
				// If it was a stale refresh, we just swallow error and return old data.
	...
	private categoryNeedsScraping(category: CategoryDocument | null): boolean {
		if (!category) return true; // Not in DB -> Scrape
		if (!category.last_scraped_at) return true; // Never scraped -> Scrape
	...
	private async bulkUpsertProducts(
		...
		try {
			...
			// Handle partial failures (common with unique constraints)
			const errorMessage =
	...
	private async getProductsForCategory(
		...
		const [products, total] = await Promise.all([
			this.productModel
				...
				.sort({ last_scraped_at: -1 }) // Show freshest first
				.skip(skip)
				.limit(pagination.limit)
				.exec(),
			this.productModel
	...
	private async findProductBySlug(
		...
		// Regex allows matching even if source_url has slight variations
	...
	async refreshProduct(slug: string): Promise<ProductWithDetail> {
		...
		if (product) {
			// Optional: Clear detail to force full clean scrape
